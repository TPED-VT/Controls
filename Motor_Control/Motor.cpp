...
void getCurrentArmFrequency(int currentArmyFrequency) {
    currentArmyFrequency = 150; 
}

void getCurrentGondolaFrequency(int currentGondolaFrequency) {
    currentGondolaFrequency = 50; 
}

int RampUp(int currentArmFrequency, int currentGondolaFrequency) {

    int targetcurrentArmFrequency = 200;
    int targetcurrentGondolaFrequency = 100;

    if (currentArmFrequency < targetcurrentArmFrequency) {
        currentArmFrequency++;
    } else if (currentArmFrequency > targetcurrentArmFrequency) {
        currentArmFrequency--;
    }

    if (currentGondolaFrequency < targetcurrentGondolaFrequency) {
        currentGondolaFrequency++;
    } else if (currentGondolaFrequency > targetcurrentGondolaFrequency) {
        currentGondolaFrequency--;
    }
    
    return currentArmFrequency + currentGondolaFrequency;
}

unsigned long getArmInterval(int currentArmFrequency) {
   if (currentArmFrequency > 0) {
         return max((unsigned long)500000L / currentArmFrequency, 10UL);
    } else {
         return 0;
   }
}
...
unsigned long getGondolaInterval(int currentGondolaFrequency) {
    if (currentGondolaFrequency > 0) {
        return max((unsigned long)500000L / currentGondolaFrequency, 10UL);
    } else {
        return 0;
    }
}
