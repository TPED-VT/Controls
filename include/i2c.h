...
int i2c_open(const char *bus_name){return 0;};

/* Initialize I2CDevice with default value */
void i2c_init_device(I2CDevice *device){};

/* Get i2c device description */
char *i2c_get_device_desc(const I2CDevice *device, char *buf, size_t size){return char();};

/* Select i2c device on i2c bus */
int i2c_select(int bus, unsigned long dev_addr, unsigned long tenbit){return 0;};
...
ssize_t i2c_read(const I2CDevice *device, unsigned int iaddr, void *buf, size_t len){return ssize_t();};
ssize_t i2c_write(const I2CDevice *device, unsigned int iaddr, const void *buf, size_t len){return ssize_t();};

/* I2c ioctl read, write can set i2c flags */
ssize_t i2c_ioctl_read(const I2CDevice *device, unsigned int iaddr, void *buf, size_t len){return ssize_t();};
ssize_t i2c_ioctl_write(const I2CDevice *device, unsigned int iaddr, const void *buf, size_t len){return ssize_t();};
